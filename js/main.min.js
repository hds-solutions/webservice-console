...
e.fadeIn();}});e.click(function(){var i=d.val(),g=e.attr("crypt").split(",");for(var h in g){switch(g[h]){case"md5":i=CryptoJS.MD5(i).toString();break;
case"base64":i=Base64.encode(i);break;default:throw new Error("Unknown crypt option "+g[h]);}}d.val(i).prop("crypted",true);e.fadeOut();});});b("#send").click(function(){a.lock();
...
a.unlock();}};if(c.token===null){delete d.headers;}$data={};b("#endpoint-params .tab-pane.active input").each(function(){if(b(this).val().toString().length>0){$data[b(this).attr("id")]=b(this).attr("json")=="true"?JSON.parse(b(this).val()):b(this).val();
